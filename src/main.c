...
/*   Updated: 2022/03/28 11:56:56 by mtiesha          ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
int	ft_handle_keypress(int keysym, t_src *s)
{
	if (65307 == keysym)
		...
		s->name = "tricorn";
	else if (101 == keysym || 113 == keysym || 120 == keysym || 122 == keysym)
		ft_scale_re(keysym, s);
...
int	main(int argc, char **argv)
{
	...
	if (argc < 2 || !ft_valid(--argc, ++argv, &s))
		ft_erroer("Error: mandelbrot || julia || burningship || tricorn");
